...
<div class="sidebar lg:overflow-auto sm:w-full lg:w-[var(--sidebar-width)] bg-gray-800 lg:h-[calc(100vh-var(--nav-height))]">
    <div class="">
        <div class="chapter py-4 px-2" v-for="(chapter, cpindex) in course.chapters" :key="chapter.id">
            <h2 class="text-xl text-gray-300 mb-2">{{ chapter.title }}</h2>
            <ul class="episodes">
                <li class="flex items-center cursor-pointer justify-between group" v-for="(episode, epindex) in chapter.episodes" :key="episode.id">
                    <div class="flex items-center">
                        ...
                        <svg class="h-6 w-6 flex-none fill-zinc-500 stroke-zinc-500 stroke-2 mr-1" stroke-linecap="round" stroke-linejoin="round">
                        <circle cx="12" cy="12" r="8"></circle>
                        </svg>
